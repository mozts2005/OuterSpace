...
class StarMapWidget(Widget):
        ...
        if hasattr(obj, 'actions'):
            ...
            for action, target, aData in obj.actions[obj.actionIndex:]:
                if target:
                    ...
                    if hasattr(trgt, 'x'):
                        self._map[self.MAP_FORDERS].append((oldX, oldY, trgt.x, trgt.y, color, getattr(obj, "isMilitary", 0), obj.oid))
    ...
    def drawFleets(self):
        ...
        if self.showFleetLines:
            for x1, y1, x2, y2, color, millitary, _ in self._map[self.MAP_FORDERS]:
